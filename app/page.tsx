  ...
  return (
    <div className="w-full min-h-screen bg-background p-4 md:px-10 lg:px-28">
      ...
      >
        <CarouselContent className="flex gap-1 bg-background">
          {CAROUSEL_IMAGES.map((item, index) => (
            <CarouselItem key={index}>
              <Card className="w-full h-[40vh] md:h-[80vh] border-none">
        ...
        <div className="flex justify-center space-x-2 my-4">
          {CAROUSEL_IMAGES.map((_, index) => (
            <div
              key={index}
              className={`w-full h-2 border-2 rounded ${current === index ? "bg-teal-500 border-teal-500" : "bg-gray-600 border-gray-600"}`}
            />
          ))}
        </div>
      </Carousel>
      ...
      <section>
        ...
        {isLoading ? (
          <div className="flex justify-center">Loading movies...</div>
        ) : (
          <>
              ...
              <PaginationContent className=" my-8">
                <PaginationItem>
                  <PaginationPrevious
                    ...
                    className={`bg-background_light hover:bg-zinc-500/80 text-zinc-500 ${currentPage === 0 ? " cursor-not-allowed": ""}`}
                  />
                </PaginationItem>
                ...
                <PaginationItem>
                  <PaginationNext
                    ...
                    className={`bg-background_light  hover:bg-zinc-400/80 text-zinc-500 ${currentPage === movieList?.data?.total_pages ? " cursor-not-allowed": ""}`}
                  />
                </PaginationItem>
              </PaginationContent>
            </Pagination>
